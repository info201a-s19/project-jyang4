...
intro_page <- tabPanel(
  ...
  h4(class = "center_text", "In our current day and age, having an effective 
    ...
    Yelp has an impressive amount of data and many of that data can be
    beneficial to new business owners. Our project takes a look at how 
    prospective businesses can use Yelp data to develop a competitive advantage.
    They can use the Yelp data to determine the best starting location, what the
    prices should be comparable to other businesses in the region, and the 
    impact of check-ins and reviews. By looking at the Yelp Data, prospective
    businesses can be more prepared and better off.
     "),
  ...
  h4(class = "center_text", "In order to answer this question, we will be using
     ...
     new and current business owners. Since the Yelp Data Set is particularly 
     large and spans across the continental United States, we thought it would 
     beneficial to focus on a small area to show the biggest impact. For
     this project, we decided to focus in the region of Las Vegas, Nevada. Our
     target reader are individuals looking to open a new business in the Las 
     Vegas region."),
